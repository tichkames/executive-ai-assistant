...
async def setup_google_oauth_provider():
    ...
    api_key = os.getenv("_LANGSMITH_API_KEY")
    if not api_key:
    ...
    if not secrets_path.exists():
        print(
            f"Error: Google OAuth client secrets file not found at {secrets_path}")
        print("Please:")
    ...
    try:
        ...
        try:
            ...
            existing_google = next(
                (p for p in providers if p.provider_id == "google"), None)

            if existing_google:
                print(
                    f"Google OAuth provider already exists: {existing_google}")
